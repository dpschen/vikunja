...
instance_and_subset () {
    ...
    DEFAULT_SUBSETTER_ARGS="--layout-features=* --unicodes=${UNICODE_RANGES[LATIN]}"
...
# Wrapper around instance_and_subset to subset by unicode range name
subset_font_variant() {
    local input_font_file="$1"
    local font_name="$2"
    local axis_settings="$3"

    local subsets=(LATIN LATIN_EXT CYRILLIC CYRILLIC_EXT GREEK GREEK_EXT HEBREW MATH SYMBOLS VIETNAMESE)
    for subset in "${subsets[@]}"; do
        instance_and_subset "$input_font_file" "$axis_settings" "${font_name}-${subset}" "--layout-features=* --unicodes=${UNICODE_RANGES[$subset]}"
    done
}
...
subset_font_variant "${ORIGINAL_FONTS}/OpenSans[wdth,wght].ttf" "OpenSans-Regular" "wdth=drop wght=400"
subset_font_variant "${ORIGINAL_FONTS}/OpenSans[wdth,wght].ttf" "OpenSans-Bold" "wdth=drop wght=700"

subset_font_variant "${ORIGINAL_FONTS}/OpenSans-Italic[wdth,wght].ttf" "OpenSans-RegularItalic" "wdth=drop wght=400"
subset_font_variant "${ORIGINAL_FONTS}/OpenSans-Italic[wdth,wght].ttf" "OpenSans-BoldItalic" "wdth=drop wght=700"
