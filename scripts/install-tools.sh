...
# Portable extraction of pnpm version from package.json:
PNPM_VERSION=$(
  grep -E '"packageManager"[[:space:]]*:[[:space:]]*"pnpm@[0-9]+\.[0-9]+\.[0-9]+"' frontend/package.json \
...
# Ensure Corepack auto-pinning is disabled
export COREPACK_ENABLE_AUTO_PIN=0  # now Corepack sees this flag :contentReference[oaicite:0]{index=0}
